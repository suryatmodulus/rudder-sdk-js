...
console.log(
  "===========  3  ===========",
  "Inside the Rudder min.js: 1st line",
  new Date().getTime()
);
...
class Analytics {
  ...
  processResponse(status, response) {
    try {
      ...
      console.log(
        "==============  10  ================",
        "after all integrations are initiated",
        new Date().getTime()
      );
    } catch (error) {
      handleError(error);
  ...
  load(writeKey, serverUrl, options) {
    ...
    // console.log(
    //   "==============  4  ================",
    //   "Just before fetching source config",
    //   new Date().getTime()
    // );
...
function processDataInAnalyticsArray(analytics) {
  if (instance.loaded) {
    for (let i = 0; i < analytics.toBeProcessedArray.length; i++) {
      ...
      console.log("=====from analytics array, calling method:: ", method);
